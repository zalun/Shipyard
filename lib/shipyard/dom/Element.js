...
var Element = new Class({
    ...
    destroy: function destroy() {
        //TODO: this should remove events and things, as well
        return this.dispose();
    },
...
function setStyle(property, value) {
    ...
    property = (property === 'float' ? getFloatName(this.node) : property);
    if (typeOf(value) !== 'string') {
        var map = (StyleMap[property] || '@').split(' ');
        if (!value.length) {
            value = [value];
        }
        value = Array.prototype.map.call(value, function(val, i){
            if (!map[i]) {
                return '';
            }
...
Element.implement({
    ...
    getComputedStyle: function getComputedStyle(property) {
        if (this.node.currentStyle) {
            return this.node.currentStyle[property];
        }
        ...
        if (computed) {
            return computed.getPropertyValue((property === floatName) ? 'float' : property);
        } else {
            return null;
        }
    },
