...
exports.overloadGetter = function(fn) {
    return function overloadedGetter(keyOrArr) {
        if (typeOf(keyOrArr) !== 'string') {
            var values = [];
            for (var i = 0, len = keyOrArr.length; i < len; i++) {
                values = fn.call(this, keyOrArr[i]);
            }
            return values;
        } else {
            return fn.call(this, keyOrArr);
        }
    };
};
